...
const ReactQuill = dynamic(async () => {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (Parchment && !Parchment.Attributor) {
      Parchment.Attributor = { Style: (Quill as any).import('attributors/style/size') };
    }
...
export default function BankSoalSection() {
  ...
  const modules = {
    ...
    imageResize: { modules: ['Resize', 'DisplaySize'], handleStyles: { backgroundColor: '#3b82f6', border: 'none', color: 'white' } }
  };
  ...
  const downloadTemplate = () => {
    const template = [
      ...
      { Nama_Bank_Soal: "UAS 2026", Mata_Pelajaran: "MTK", Tipe_Soal: "isian", Pertanyaan: "<p>12 x 12 = ...</p>", Jawaban_Benar: "144", Bobot: 10 }
    ];
    ...
    reader.onload = async (evt) => {
      try {
        ...
        data.forEach((item: any) => {
          ...
          batch.set(docRef, {
            namaBankSoal: item.Nama_Bank_Soal, mapel: item.Mata_Pelajaran, tipe: tipe, pertanyaan: item.Pertanyaan,
            ...
            jawabanBenar: String(item.Jawaban_Benar), bobot: Number(item.Bobot || 1), createdAt: new Date(), updatedAt: new Date()
          });
        });
        await batch.commit();
  ...
  const handleEditKelompok = (k: any) => {
    setNewKelompok({ id: k.id, nama: k.nama });
    setIsEditKelompok(true);
  };

  const hapusKelompok = async (id: string) => {
    if (confirm("Hapus kelompok ini?")) await deleteDoc(doc(db, "kelompok_soal", id));
  };
  ...
  const simpanSoal = async () => {
    ...
    setLoading(true);
    const payload = {
      namaBankSoal, pertanyaan, mapel: selectedMapel, tipe: tipeSoal, bobot: Number(bobot),
      opsi: tipeSoal === 'pilihan_ganda' ? opsi : null, jawabanBenar: finalKunci, media, updatedAt: new Date()
    };
    try {
    ...
    <div className="max-w-7xl mx-auto pb-20 animate-in fade-in duration-500">
              ...
              <div key={s.id} className="bg-white p-4 rounded-2xl border shadow-sm flex flex-col md:flex-row items-start md:items-center gap-4 transition-all hover:border-blue-200">
                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-[9px] font-black text-blue-600 uppercase bg-blue-50 px-2 py-0.5 rounded">{s.namaBankSoal}</span>
                    <span className="text-[9px] font-bold text-slate-400 uppercase">{s.mapel}</span>
                    {s.pertanyaan?.includes('<img') && <ImageIcon size={14} className="text-purple-400" />}
                  </div>
                ...
                <div className="flex items-center gap-6 w-full md:w-auto pt-3 md:pt-0 border-t md:border-t-0 border-slate-50 justify-between md:justify-end">
                  <div className="flex flex-col items-start md:items-center">
                    ...
                    <span className="text-xs font-black text-green-600 uppercase">{s.tipe === 'pencocokan' ? 'Match' : s.jawabanBenar}</span>
                  </div>
                  <div className="flex flex-col items-start md:items-center">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => confirm('Hapus?') && deleteDoc(doc(db, "bank_soal", s.id))} className="p-2.5 bg-red-50 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all"><Trash2 size={16}/></button>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="bg-white p-8 md:p-12 rounded-[3rem] border-2 border-slate-100 shadow-sm space-y-8">
            ...
            <div className="space-y-4 pt-6 border-t-2 border-slate-100">
              {tipeSoal === 'pilihan_ganda' && (
                <div className="grid grid-cols-1 gap-3">
                  ...
                  <div className="mt-4 p-4 bg-green-50 rounded-2xl border-2 border-green-100">
                    <label className="text-[10px] font-black text-green-600 uppercase tracking-widest ml-1">Kunci Jawaban</label>
                    <div className="flex gap-2 mt-2">
                      {['a','b','c','d','e'].map(l => (
                        <button key={l} type="button" onClick={() => setJawabanBenar(l)} className={`flex-1 py-4 rounded-xl font-black text-xs border-2 transition-all ${jawabanBenar === l ? 'bg-green-600 border-green-600 text-white shadow-lg shadow-green-100' : 'bg-white text-slate-300 border-slate-200'}`}>{l.toUpperCase()}</button>
                      ))}
                    </div>
                  </div>
                </div>
              ...
              {(tipeSoal === 'isian' || tipeSoal === 'uraian') && (
                <input placeholder="Kunci jawaban..." className="w-full p-5 bg-green-50 border-2 border-green-100 rounded-2xl text-sm font-bold text-green-700 outline-none" value={jawabanBenar} onChange={(e) => setJawabanBenar(e.target.value)} />
              )}
              {tipeSoal === 'pencocokan' && (
                <div className="grid grid-cols-1 gap-3">
                  {pairs.map((p, idx) => (
            ...
            <div className="flex gap-4 items-end pt-10">
              <div className="w-32">
                <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Bobot</label>
              ...
              <button onClick={simpanSoal} disabled={loading} className={`flex-1 py-5 rounded-[2rem] font-black text-xs uppercase tracking-widest text-white shadow-2xl transition-all ${isEditMode ? 'bg-orange-500 shadow-orange-200' : 'bg-blue-600 shadow-blue-200'}`}>
      ...
      {showKelompokModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-xl rounded-[3rem] p-8 shadow-2xl space-y-6 border-2 border-slate-100 flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter">Kelola Kelompok</h3>
              <button onClick={() => { setShowKelompokModal(false); setIsEditKelompok(false); setNewKelompok({ id: '', nama: '' }); }} className="p-2 bg-slate-100 rounded-full hover:text-red-500 transition-all"><X size={20}/></button>
            </div>
            
            <div className="flex gap-3">
              <input placeholder="Nama Kelompok Baru..." className="flex-1 p-4 bg-slate-50 border-2 border-slate-200 rounded-2xl text-sm font-bold outline-none focus:border-blue-400" value={newKelompok.nama} onChange={(e) => setNewKelompok({...newKelompok, nama: e.target.value})} />
              <button onClick={simpanKelompok} className="bg-blue-600 text-white px-6 rounded-2xl font-black text-xs uppercase hover:bg-blue-700 shadow-lg shadow-blue-100 flex items-center gap-2 transition-all">
                {isEditKelompok ? <Save size={18}/> : <Plus size={18}/>} {isEditKelompok ? 'Update' : 'Tambah'}
              </button>
            </div>

            <div className="flex-1 overflow-y-auto space-y-2 pr-2">
              <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Daftar Kelompok Terdaftar</p>
              {kelompokList.map((k) => (
                <div key={k.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-blue-200 transition-all group">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => handleEditKelompok(k)} className="p-2 text-orange-500 hover:bg-orange-50 rounded-xl transition-all"><Edit3 size={16}/></button>
                    <button onClick={() => hapusKelompok(k.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-xl transition-all"><Trash2 size={16}/></button>
                  </div>
                </div>
              ))}
            </div>
